...
`).trim():""}function og(){const[e,t]=C.useState(!1),[n,i]=C.useState(-1),[o,a]=C.useState([]),r=C.useRef([]),l=C.useRef(null),s=C.useCallback(()=>{const m=speechSynthesis.getVoices().filter(u=>u.lang.startsWith("it"));return m.find(u=>u.name.includes("Emma"))||m.find(u=>u.name.includes("Enhanced")||u.name.includes("Premium"))||m[0]||null},[]),d=C.useCallback(()=>{speechSynthesis.cancel(),r.current=[],t(!1),i(-1)},[]),y=C.useCallback((F,m=0)=>{speechSynthesis.cancel(),r.current=F.slice(m),a(F);const u=()=>{if(r.current.length===0){t(!1),i(-1);return}const f=r.current.shift(),S=F.indexOf(f);i(S);const w=`${f.title}. ${ig(f.text)}`,x=new SpeechSynthesisUtterance(w);x.lang="it-IT",x.rate=1;const P=s();P&&(x.voice=P),x.onend=u,x.onerror=u,l.current=x,speechSynthesis.speak(x)};t(!0),u()},[s]),v=C.useCallback(()=>{speechSynthesis.pause(),t(!1)},[]),h=C.useCallback(()=>{speechSynthesis.resume(),t(!0)},[]),b=C.useCallback(()=>{speechSynthesis.cancel()},[]),z=C.useCallback((F,m,u,f)=>{const S=[];F&&S.push({title:"Cross-Synthesis",text:F,type:"synthesis"});for(const w of u)m[w.id]&&!m[w.id].startsWith("Errore")&&S.push({title:w.name,text:m[w.id],type:"thinker"});if(f&&f.length>0){const w=f.map(x=>{var P;return`${x.name}: ${x.description}${(P=x.citedBy)!=null&&P.length?`. Citato da ${x.citedBy.join(", ")}`:""}`}).join(`.

`);S.push({title:"Progetti Emergenti",text:w,type:"projects"})}y(S)},[y]),T=C.useCallback((F,m)=>{y([{title:F,text:m,type:"single"}])},[y]);return C.useEffect(()=>()=>speechSynthesis.cancel(),[]),{isPlaying:e,currentChapter:n,chapters:o,playAll:z,playSingle:T,pause:v,resume:h,stop:d,skipForward:b}}const nr=Wn?`?theme=${Wn}`:"",ag=`
