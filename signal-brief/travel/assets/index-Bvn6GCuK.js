 ...
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */var wp=C,Tp=Symbol.for("react.element"),Cp=Symbol.for("react.fragment"),Pp=Object.prototype.hasOwnProperty,Ap=wp.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner,Mp={key:!0,ref:!0,__self:!0,__source:!0};function Lc(e,t,n){var i,o={},a=null,r=null;n!==void 0&&(a=""+n),t.key!==void 0&&(a=""+t.key),t.ref!==void 0&&(r=t.ref);for(i in t)Pp.call(t,i)&&!Mp.hasOwnProperty(i)&&(o[i]=t[i]);if(e&&e.defaultProps)for(i in t=e.defaultProps,t)o[i]===void 0&&(o[i]=t[i]);return{$$typeof:Tp,type:e,key:a,ref:r,props:o,_owner:Ap.current}}Zo.Fragment=Cp;Zo.jsx=Lc;Zo.jsxs=Lc;Ic.exports=Zo;var c=Ic.exports,jc={exports:{}},Ke={},_c={exports:{}},Dc={};/**
 * @license React
...
`).trim():""}function ng(){const[e,t]=C.useState(!1),[n,i]=C.useState(-1),[o,a]=C.useState([]),r=C.useRef([]),l=C.useRef(null),s=C.useCallback(()=>{const m=speechSynthesis.getVoices().filter(u=>u.lang.startsWith("it"));return m.find(u=>u.name.includes("Emma"))||m.find(u=>u.name.includes("Enhanced")||u.name.includes("Premium"))||m[0]||null},[]),d=C.useCallback(()=>{speechSynthesis.cancel(),r.current=[],t(!1),i(-1)},[]),y=C.useCallback((F,m=0)=>{speechSynthesis.cancel(),r.current=F.slice(m),a(F);const u=()=>{if(r.current.length===0){t(!1),i(-1);return}const f=r.current.shift(),S=F.indexOf(f);i(S);const k=`${f.title}. ${tg(f.text)}`,x=new SpeechSynthesisUtterance(k);x.lang="it-IT",x.rate=1;const P=s();P&&(x.voice=P),x.onend=u,x.onerror=u,l.current=x,speechSynthesis.speak(x)};t(!0),u()},[s]),v=C.useCallback(()=>{speechSynthesis.pause(),t(!1)},[]),h=C.useCallback(()=>{speechSynthesis.resume(),t(!0)},[]),b=C.useCallback(()=>{speechSynthesis.cancel()},[]),z=C.useCallback((F,m,u,f)=>{const S=[];F&&S.push({title:"Cross-Synthesis",text:F,type:"synthesis"});for(const k of u)m[k.id]&&!m[k.id].startsWith("Errore")&&S.push({title:k.name,text:m[k.id],type:"thinker"});if(f&&f.length>0){const k=f.map(x=>{var P;return`${x.name}: ${x.description}${(P=x.citedBy)!=null&&P.length?`. Citato da ${x.citedBy.join(", ")}`:""}`}).join(`.

`);S.push({title:"Progetti Emergenti",text:k,type:"projects"})}y(S)},[y]),T=C.useCallback((F,m)=>{y([{title:F,text:m,type:"single"}])},[y]);return C.useEffect(()=>()=>speechSynthesis.cancel(),[]),{isPlaying:e,currentChapter:n,chapters:o,playAll:z,playSingle:T,pause:v,resume:h,stop:d,skipForward:b}}const nr=Wn?`?theme=${Wn}`:"",ig=`
