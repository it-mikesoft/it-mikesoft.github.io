...
`),o=n&&n.length>0?n.join(", "):"GitHub Trending, trendshift.io, Hacker News",l=pl(fl.projects,{signals:r,synthesis:t,sources_list:o}),a=((await ml(l)).text||"").trim().match(/\[[\s\S]*\]/);if(!a)throw new Error("Formato risposta non valido");return JSON.parse(a[0])}async function Am(e,t){const n=pl(fl.conceptDiff,{old_synthesis:e,new_synthesis:t}),l=((await ml(n)).text||"").trim().match(/\{[\s\S]*\}/);if(!l)throw new Error("Formato risposta non valido");return JSON.parse(l[0])}let hu=!1;async function $m(){if(hu)return;if(!(await fetch(`${bd}/login`,{method:"POST",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify({email:"signal@stelle.local",password:"signal-brief-2026"})})).ok)throw new Error("Login Summon Stars fallito");hu=!0}async function Fm(e,t,n){await $m();const r=t?`Contesto (cross-synthesis attuale):
${t}
...
`).trim():""}function Km(){const[e,t]=I.useState(!1),[n,r]=I.useState(-1),[o,l]=I.useState([]),i=I.useRef([]),s=I.useRef(null),a=I.useCallback(()=>{const p=speechSynthesis.getVoices().filter(c=>c.lang.startsWith("it"));return p.find(c=>c.name.includes("Emma"))||p.find(c=>c.name.includes("Enhanced")||c.name.includes("Premium"))||p[0]||null},[]),f=I.useCallback(()=>{speechSynthesis.cancel(),i.current=[],t(!1),r(-1)},[]),y=I.useCallback(($,p=0)=>{speechSynthesis.cancel(),i.current=$.slice(p),l($);const c=()=>{if(i.current.length===0){t(!1),r(-1);return}const m=i.current.shift(),x=$.indexOf(m);r(x);const E=`${m.title}. ${Qm(m.text)}`,w=new SpeechSynthesisUtterance(E);w.lang="it-IT",w.rate=1;const P=a();P&&(w.voice=P),w.onend=c,w.onerror=c,s.current=w,speechSynthesis.speak(w)};t(!0),c()},[a]),v=I.useCallback(()=>{speechSynthesis.pause(),t(!1)},[]),g=I.useCallback(()=>{speechSynthesis.resume(),t(!0)},[]),S=I.useCallback(()=>{speechSynthesis.cancel()},[]),N=I.useCallback(($,p,c,m)=>{const x=[];$&&x.push({title:"Cross-Synthesis",text:$,type:"synthesis"});for(const E of c)p[E.id]&&!p[E.id].startsWith("Errore")&&x.push({title:E.name,text:p[E.id],type:"thinker"});if(m&&m.length>0){const E=m.map(w=>{var P;return`${w.name}: ${w.description}${(P=w.citedBy)!=null&&P.length?`. Citato da ${w.citedBy.join(", ")}`:""}`}).join(`.

`);x.push({title:"Progetti Emergenti",text:E,type:"projects"})}y(x)},[y]),_=I.useCallback(($,p)=>{y([{title:$,text:p,type:"single"}])},[y]);return I.useEffect(()=>()=>speechSynthesis.cancel(),[]),{isPlaying:e,currentChapter:n,chapters:o,playAll:N,playSingle:_,pause:v,resume:g,stop:f,skipForward:S}}const Gm=`
